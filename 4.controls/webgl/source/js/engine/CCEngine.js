...
CCEngine.prototype.updateOnce = function()
{
    ...
    {
		// TODO: Controls - move to Controls class?
		var touch = this.touch;
		if( touch.touching != false )
		{
			touch.timeHeld += delta;
			
			if( touch.touching == touch_pressed )
			{
				this.handleControls( gUI, touch_pressed );
			}
		}
...
CCEngine.prototype.handleControls = function(ui, state)
{
	var returnValue = false;
	
	var table = this.table;
	var touchOverTable = ui.isMouseOverObject( table );
	this.touchUpdate( ui, state );
	...
	{
		var scene = scenes[i];
		returnValue |= scene.handleControls( ui, state, this.touch, touchOverTable );
        if( returnValue )
        {
            break;
        }
	...
	if( ui.allowDefaults )
	{
		returnValue = ui.allowDefaults = false;
	}
	return returnValue;
}


CCEngine.prototype.touchUpdate = function(ui, state)
{
    var touch = this.touch;
	
    var canvas = this.renderer.canvas;
    var x = ( ui.x - canvas.offsetLeft ) / canvas.clientWidth;
    var y = ( ui.y - canvas.clientTop ) / canvas.clientHeight;
